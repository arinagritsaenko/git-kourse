...
$(document).ready(function() {
    ...
    $('.global-header .toggle-user-dropdown, .global-header .toggle-user-dropdown span').click(function(e) {
        ...
        var $icon = $('.global-header .set-active');
    ...
    $('.mobile-menu-container').click(function() {
        ...
        $hamburgerMenuWrap.removeClass('mobile-menu-open');
    ...
    $('.global-header .hamburger-menu-wrap').click(function() {
        $hamburgerMenu = $('.global-header .hamburger-menu');
        $mobileMenu = $('.mobile-menu');
        $mobileMenuContainer = $('.mobile-menu-container');
        $hamburgerMenuWrap = $('.hamburger-menu-wrap');
        $body = $('body');
        if ($mobileMenu.is(':visible')) {
            ...
            $hamburgerMenuWrap.addClass('mobile-menu-open');
    ...
    if ($('.mobile-nav-item').size() === 0) {
        $('.global-header .hamburger-menu .mobile-menu-container').addClass('hidden');
    }
...
$('.mobile-menu, .global-header .mobile-menu-container').on('keydown', function(e) {
    'use strict';
    var isNext,
        nextLink,
        loopFirst,
    ...
    if ((e.key === 'Enter' || e.key === 'Space') && (isToggle || isHamburgerMenu)) {
        ...
        $curTarget.click();
        if (isHamburgerMenu) {
            if ($('.mobile-menu .mobile-menu-container').is(':visible')) {
                $hamburgerMenu.attr('aria-expanded', true);
                $('.mobile-menu .mobile-nav-link a .mobile-menu-container').first().focus();
            } else {
                $hamburgerMenu.attr('aria-expanded', false);
            }
        } else if (isToggle) {
    ...
    if ((e.key === 'ArrowUp' || e.key === 'ArrowDown') && (isDropdownOption || isMobileOption ||
        ...
        if (isNext && !isHamburgerMenu && !isToggle && isLastItem) {
            ...
            nextLink = isHamburgerMenu ? $('.mobile-menu .mobile-nav-link a .mobile-menu-container').last()
                : $('.global-header .dropdown-user-menu .dropdown-nav-item').last().find('a');
        } else if (isNext && (isHamburgerMenu || isToggle)) {
            // Loop to the first element from the menu icon
            nextLink = isHamburgerMenu ? $('.mobile-menu .mobile-nav-link a .mobile-menu-container').first()
    ...
    if ((e.key === 'Tab') && ((isDropdownOption && isLastItem) || (isMobileOption && isLastItem) || (isHamburgerMenu
        ...
        if (isDropdownOption || isToggle) {
            ...
            nextLink = loopFirst ? $hamburgerMenu : $('.mobile-menu .mobile-nav-link a .mobile-menu-container').last();
        }
        nextLink.focus();
    }
});
